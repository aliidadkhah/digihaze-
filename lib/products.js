...
function getReadClient() {
  ...
  // نکته مهم: روی هاست ما (vinext روی Cloudflare Workers)، fetch به‌صورت
  // پیش‌فرض توسط Data Cache (که با KV پیاده‌سازی شده) کش می‌شود؛ این باعث
  // می‌شد بعد از تغییر محصول/اسلاگ در Supabase، صفحات همچنان داده‌ی قدیمی
  // نشون بدن. با پاس دادن cache:"no-store" این کش رو برای درخواست‌های
  // Supabase غیرفعال می‌کنیم تا داده همیشه تازه باشد.
...
function mapRow(row) {
  ...
  return {
    ...
    // اگر رنگی برای محصول ثبت نشده باشد (مثلا محصولات قدیمی)، یک رنگ پیش‌فرض
    // استفاده می‌شود تا برچسب و هاور کارت بی‌رنگ نمانند
    color:
      row.color || "#0A84FF",

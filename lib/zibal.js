...
export async function zibalVerify(trackId) {
  ...
  // نکته مهم: زیبال انتظار داره trackId به صورت عدد (Number) توی JSON
  // ارسال بشه، نه رشته. چون این مقدار از query string مرورگر میاد
  // (که همیشه string هست)، اگه بدون تبدیل فرستاده بشه، زیبال درخواست
  // رو نامعتبر تلقی می‌کنه و حتی برای تراکنش‌های واقعاً موفق هم
  // نتیجه‌ی ناموفق برمی‌گردونه.
  const numericTrackId = Number(trackId);

  if (!Number.isFinite(numericTrackId)) {
    throw new Error(`trackId نامعتبر است: ${trackId}`);
  }
